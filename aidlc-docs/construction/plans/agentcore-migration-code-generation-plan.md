...
| コンポーネント | 機能 |
|--------------|------|
| AgentCore Memory | セッション状態統合管理（会話履歴、メトリクス） |
...
- [x] 1.1 `cdk/lib/constructs/agentcore/agentcore-runtime.ts` 作成
  - L2 Construct使用（@aws-cdk/aws-bedrock-agentcore-alpha）
  - IAMロール設定
  - AgentCore Identity設定（JWT認証）
  - 環境変数設定
  - AgentCore Memory ID環境変数対応

#### Step 1.5: AgentCore Memory CDKコンストラクト作成
- [x] 1.5.1 `cdk/lib/constructs/agentcore/agentcore-memory.ts` 作成
  - L2 Construct使用（@aws-cdk/aws-bedrock-agentcore-alpha）
  - STM（Short-Term Memory）設定
  - 90日間保持期間

#### Step 2: 評価画面API CDKコンストラクト作成
- [x] 2.1 `cdk/lib/constructs/api/evaluation-api.ts` 作成
  - Lambda関数定義
  - API Gatewayエンドポイント定義
  - IAMロール設定

#### Step 3: InfrastructureStack更新
- [x] 3.1 `cdk/lib/infrastructure-stack.ts` 更新
  - AgentCore Memoryコンストラクト追加
  - AgentCore Runtimeコンストラクト追加（Memory ID連携）
  - 評価画面APIコンストラクト追加
  - 旧Lambda/API Gateway削除
  - DynamoDBテーブル削除（会話履歴、メトリクス）
  - **注**: 既存Apiコンストラクトとの統合が複雑なため、Build and Testステージで段階的に実施

### Part 2: エージェントコード生成

#### Step 4: NPC会話エージェント作成
- [x] 4.1 `cdk/agents/npc-conversation/agent.py` 作成
  - FastAPI + Strands Agents エントリーポイント
  - AgentCore Memory Session Manager統合
  - Bedrock Claude 4.5 Haiku呼び出し
  - 感情状態管理
- [x] 4.2 `cdk/agents/npc-conversation/requirements.txt` 更新
  - bedrock-agentcore[strands-agents] 追加

#### Step 5: スコアリングエージェント作成
- [x] 5.1 `cdk/agents/realtime-scoring/agent.py` 作成
  - FastAPI + Strands Agents エントリーポイント
  - メトリクス計算ロジック
  - ゴール評価ロジック
  - AgentCore Memory CreateEvent（メトリクス保存）
- [x] 5.2 `cdk/agents/realtime-scoring/requirements.txt` 更新
  - bedrock-agentcore[strands-agents] 追加

#### Step 6: フィードバック分析エージェント作成
- [x] 6.1 `cdk/agents/feedback-analysis/agent.py` 作成
  - FastAPI + Strands Agents エントリーポイント
  - AgentCore Memory ListEvents呼び出し（会話履歴取得）
  - フィードバック生成ロジック
  - S3保存
- [x] 6.2 `cdk/agents/feedback-analysis/requirements.txt` 更新
  - bedrock-agentcore[strands-agents] 追加

#### Step 7: 動画分析エージェント作成
- [x] 7.1 `cdk/agents/video-analysis/agent.py` 作成
  - FastAPI + Strands Agents エントリーポイント
  - S3動画取得
  - Nova Premiere呼び出し（boto3 converse - マルチモーダル対応）
  - S3結果保存

#### Step 8: 音声分析エージェント作成
- [x] 8.1 `cdk/agents/audio-analysis/agent.py` 作成
  - FastAPI + Strands Agents エントリーポイント
  - S3音声取得
...
- [x] 9.1 `cdk/lambda/evaluation-api/index.py` 作成
...
- [x] 11.1 `frontend/src/services/AgentCoreService.ts` 作成
...
#### Step 15: 環境変数設定
- [x] 15.1 `frontend/.env` 更新
  - AgentCore Runtime ARN追加（コメントアウト状態）
- [ ] 15.2 `cdk/cdk.json` 更新
  - AgentCore設定をcontextに追加（Build and Testステージで実施）
...
| `cdk/lib/constructs/agentcore/agentcore-memory.ts` | 新規作成 |
| `cdk/lib/constructs/api/evaluation-api.ts` | 新規作成 |
| `cdk/lib/infrastructure-stack.ts` | 更新 |
...
**作成日**: 2026-01-08
**更新日**: 2026-01-09
**ステータス**: AgentCore Memory統合デプロイ完了 - テスト待ち

## デプロイ結果 (2026-01-09)

### AgentCore Memory
- **Memory ID**: `dev_session_memory-pLEHVU4YHf`
- **Memory ARN**: `arn:aws:bedrock-agentcore:us-west-2:241231359358:memory/dev_session_memory-pLEHVU4YHf`

### AgentCore Runtimes
| エージェント | ARN |
|------------|-----|
| NPC会話 | `arn:aws:bedrock-agentcore:us-west-2:241231359358:runtime/dev_npc_conversation_l2-TR4z3YHHP0` |
| リアルタイムスコアリング | `arn:aws:bedrock-agentcore:us-west-2:241231359358:runtime/dev_realtime_scoring_l2-WAXsUoDIvh` |
| フィードバック分析 | `arn:aws:bedrock-agentcore:us-west-2:241231359358:runtime/dev_feedback_analysis_l2-caYCkE8Qgh` |
| 動画分析 | `arn:aws:bedrock-agentcore:us-west-2:241231359358:runtime/dev_video_analysis_l2-qMsjg6Gy3I` |
| 音声分析 | `arn:aws:bedrock-agentcore:us-west-2:241231359358:runtime/dev_audio_analysis_l2-Z5krI96uNM` |
